...
(defn add-drink [{:keys [params session]}]
  (cond
    ...
    (redirect (str "/drink/" (add-drink->db params)))
    :else
    (->
...
(defn get-drink [{:keys [params session]} & [message]]
  (cond
    ...
    :else
    (get-drink-page "templates/drink.html" params session message)))
...
(defresource update-drink [{:keys [params session]}]
  :allowed-methods [:put]
  :available-media-types ["application/json"]
  :malformed? (fn [_] (not (drink-validation? params)))
