...
(defn search-drinks [text]
  (k/select drink
            ...
            (k/where (or
                       {:id text}
                       {:name [reservation (get-text-search text)]}
                       {:drink_type.name [reservation (get-text-search text)]}
                       {:alcohol text}
                       {:country [reservation (get-text-search text)]}
                       {:manufacturer [reservation (get-text-search text)]}))
            (k/order :id :ASC)))
